import{_ as n}from"./plugin-vue_export-helper-x3n3nnut.js";import{o as s,c as a,e as t}from"./app-picjwZeR.js";const e={},p=t(`<h2 id="开启立即监听-响应式数据的引用问题" tabindex="-1"><a class="header-anchor" href="#开启立即监听-响应式数据的引用问题" aria-hidden="true">#</a> 开启立即监听，响应式数据的引用问题</h2><h3 id="问题描述" tabindex="-1"><a class="header-anchor" href="#问题描述" aria-hidden="true">#</a> 问题描述</h3><p>我的需求是监听 tab 栏的变化，去请求不同的列表。并且需要在初始阶段就请求一次列表，所以需要开启立即监听。</p><blockquote><p>想省去 onMounted</p></blockquote><p>但是开启立即监听后，出现了 <code>Cannot access &#39;searchForm&#39; before initialization</code> 的报错。</p><p>这是因为在 watch hook 函数中引用了 searchForm 这个响应式数据，而该数据定义在 watch 后面，如果不开立即监听就没关系，一旦开启立即监听后，就会报错。</p><div class="language-typescript line-numbers-mode" data-ext="ts"><pre class="language-typescript"><code><span class="token keyword">const</span> tabName <span class="token operator">=</span> <span class="token function">ref</span><span class="token punctuation">(</span><span class="token string">&quot;ON_SHELF&quot;</span><span class="token punctuation">)</span>
