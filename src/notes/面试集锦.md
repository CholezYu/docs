...
- 更新用法：
  ...
  - Vue2 的 v-model 绑定的是 `:value` 和 `@input`；

    Vue3 的 v-model 绑定的是 `:modelValue` 和 `@update:modelValue`。
...
- 删除旧语法：
  ...
  - 移除了 `.native` 修饰符，未被 defineEmits 接收的事件会被视为原生事件。
...
- 实现原理：
  ...
  - 如果是输入框或文本域，绑定 `:value` 和 `@input`；

  - 如果是单选框或多选框，绑定 `:checked` 和 `@change`；

  - 如果是下拉列表，绑定 `:value` 和 `@change`；

  - 如果是组件，Vue2 绑定 `:value` 和 `@input`，Vue3 绑定 `:modelValue` 和 `@update:modelValue`。
...
[异步组件 | Vue](../front-end/Vue.md#异步组件)

[缓存组件 | Vue](../front-end/Vue.md#缓存组件)
