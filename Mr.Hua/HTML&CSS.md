...
2. 加载顺序不同 页面在加载的时候，link 引入的 css 会同时被加载 @import 引入的 css 只有等页面全部下载完成以后才进行加载，所以可能会出现闪烁

3. 兼容性 link 所有浏览器都支持 @import 低版本 ie 不支持

4. 使用 JS 控制样式 JS 只能控制 link 标签 @import 是不能够被 JS 控制的

5. style 标签和 link 标签的书写位置
...
3. 引入外部字体包:
   ...
   使用 @font-face 给项目引入外部字体

   - src:url(): 引入字体包

   - font-family: 给当前字体起一个名字

2. 可以在 font-family 中直接使用即可

4. font-style: 控制文字风格(是否倾斜)
