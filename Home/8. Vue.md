...
- 观察者模式中的观察者被称为 "订阅者"，观察者模式中的目标对象被称为 "发布者"
...
- 详细过程：
  ...
  3. 在 Dep 类（订阅中心）中，有收集所有 watcher 的方法，和通知所有 watcher 更新数据的方法。当 _data 中每一个属性被劫持的时候，会创建一个 dep（Dep 的实例化对象），在 getter 中调用 dep 的 depend 方法收集依赖，在 setter 中调用 dep 的 notify 方法通知更新
