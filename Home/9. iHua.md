...
- 对于所有节点条件渲染：使用计算属性，返回过滤后的列表，再对列表中的元素进行渲染
...
3. 初始化数据注入和数据劫持，同时初始化 data methods computed watch 等
...
- 挂载之后，`mounted` 触发，视图呈现的是 Vue 编译后的 DOM。我们一般在这个阶段进行**开启定时器、发送网络请求、订阅消息、监听自定义事件**等操作
...
当 `$destroy` 被调用，或条件渲染时组件或路由被销毁时，进入销毁阶段：

- 实例销毁之前，`beforeDestroy` 触发，我们一般在这个阶段进行**关闭定时器、取消订阅、移除自定义事件**等操作
