...
> 视图呈现的是未被解析的模板。DOM 操作最终都无效，因为 Vue 即将把虚拟 DOM 转为真实 DOM 插入页面
>
...
>
> 一般在这个阶段进行 **开启定时器、发送网络请求、订阅消息、监听自定义事件** 等操作

## 更新流程

> 数据改变，视图未更新 => `beforeUpdate` => 虚拟 DOM 重新渲染并与旧的虚拟 DOM 比较 => `updated`
...
完全销毁一个实例，断开与子组件的连接，解绑全部指令及自定义事件监听器
...
>
> 一般在这个阶段进行 **关闭定时器、取消订阅、移除自定义事件** 等操作

### destroyed

实例销毁之后被调用

> 实例中的所有指令都被解绑，所有的自定义事件监听器被移除，所有的子组件也都被销毁
