...
>
> 所以只要访问的 computed 没有发生改变，多次访问会返回缓存的计算结果；而多次调用 methods 会重新执行函数
...
侦听器默认只能侦听 data 中的属性本身，不能侦听其内部的属性
...
watch: {
  numbers: {
    handler() { /* 当 numbers 本身变化时才执行，只有 count 发生改变不会执行 */ }
  }
}
```
...
`vm.$watch(expression/function, callback, [options])`

与 watch 使用方式相同

```js
vm.$watch("count", function(value, oldValue) { /* ... */ })
...
`immediate: true` - 侦听开始后(初始化渲染)立即调用 handler，默认为 false
...
`deep: true` - 侦听 data 中的属性与其内部的属性，默认不能侦听 data 中的属性内部的属性
...
将动态绑定的对象定义在 HTML 标签内

```html
<div :class="{ active: isActive, visible: isVisible }"></div>
```
...
```html
<div :style="{ color: activeColor, fontSize: fSize + 'px' }"></div>
```
...
v-if 初始渲染表达式的值为"真"的元素，并显示这些元素，在切换时销毁或创建 DOM 元素
...
v-show 初始渲染所有元素，但只显示表达式的值为"真"的元素，在切换时显示或隐藏 DOM 元素 
...
用 \<template> 条件渲染多个元素，最终的渲染结果不包含 \<template>
...
```html
<li v-for="item in colors"></li>
```

`(item, index) in array`

```html
<li v-for="(item, index) in colors"></li>
```
...
```html
<li v-for="(value, key) in person"></li>
```
...
在使用 v-for 时为遍历的每一项提供唯一的 key，使它能跟踪每个节点，重新排序时可以复用现有元素
...
用 \<template> 循环渲染多个元素，最终的渲染结果不包含 \<template>
...
<!-- param -->
<button @click="click(param)"></button>

<!-- event -->
<button @click="click($event)"></button>
```

传递两个参数 - `click(param1, param2)`
...
<!-- param param -->
<button @click="click(param1, param2)"></button>
...
`keyup.{keyCode | keyAlias}` - 触发 "按键码" 或 "按键别名" 对应的按键时触发回调
...
将输入的字符串转为有效的数字 (类似 parseFloat)，通常配合 `type="number"` 使用
...
更新元素的 textContent。如果要更新部分 textContent，需要使用 Mustache
...
更新元素的 innerHTML。将 data 对象中的数据按照 HTML 格式解析并渲染
...
与 CSS `[v-cloak] { display: none }` 一起使用，可以隐藏未编译的 Mustache，指令在编译结束后消失
...
只渲染一次元素和组件，随后的重新渲染，元素和组件及所有子节点将被视为静态内容并跳过

## v-pre

跳过这个元素和其子元素的编译过程，可以用来跳过没有指令或 Mustache 的节点，加快编译
...
> new Vue => 初始化事件和生命周期 => beforeCreate => 初始化数据侦听和数据代理 => created
...
> 解析模板，生成虚拟 DOM => beforeMount => 创建 vm.$el 并替换 el => mounted
>
> > 创建 vm.$el 并替换 el：将虚拟 DOM 转为真实 DOM 插入页面，此时挂载完毕
...
> 页面呈现的是未经 Vue 编译的 DOM
>
> 所有 DOM 操作最终都无效，因为 Vue 即将把虚拟 DOM 转为真实 DOM 插入页面
...
>
> DOM 操作均有效（尽量避免），可以进行 **开启定时器、发送网络请求、订阅消息、监听自定义事件** 等操作

## 更新流程

> 数据发生改变 => beforeUpdate => 虚拟 DOM 重新渲染并与旧的虚拟 DOM 进行比较 => updated

### beforeUpdate

数据发生改变之后，真实 DOM 更新之前被调用

> 数据已更新，页面未更新

### updated

数据发生改变导致的虚拟 DOM 重新渲染和更新完毕之后被调用
...
完全销毁一个实例，清理与其它实例（子组件）的连接，解绑全部指令及自定义事件监听器

触发 beforeDestroy 和 destroyed 的钩子

>尽量避免调用该方法，最好使用 v-if 和 v-for 以数据驱动的方式控制子组件的生命周期

### beforeDestroy★

实例销毁之前被调用

> 实例中的 data、methods、指令等都可用，可以进行 **关闭定时器、取消订阅、移除自定义事件** 等操作

### destroyed

实例销毁之后被调用

> 实例中的所有指令都被解绑，所有的自定义事件监听器被移除，所有的子实例也都被销毁
...
```html
<!-- 不传值默认为 true -->
<Child isfemale></Child>

<Child :isfemale="false"></Child>
<Child :isfemale="child.isfemale"></Child>
```
