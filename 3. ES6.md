...
Promise 是异步编程的新的解决方案，比传统的解决方案（回调函数和自定义事件）更强大

### 具体表达

从语法上来说：Promise 是一个构造函数

从功能上来说：Promise 对象用来封装异步操作
...
> Promise 只有两种状态改变，只要这两种情况发生，状态就凝固了，此时称为 resolved（已定型）

- pending  =>  fulfilled：异步操作成功
...
const promise = new Promise((resolve, reject) => {
  // ...
  if (/* 异步操作成功 */) {
    resolve(value)  // value 作为参数传给成功的回调
  } else /* 异步操作失败 */ {
    reject(error)  // error 作为参数传给失败的回调
  }
})
...
- Promise：启动异步任务 => 返回 Promise 对象 => 指定回调函数（可以在异步任务结束后指定）
...
/* then 方法指定的回调函数在当前脚本所有同步任务执行完成后才会执行 */
promise.then(() => {
...
- 回调地狱：回调函数嵌套调用，外部回调函数异步执行的结果时嵌套的回调函数执行的条件

- 回调地狱的缺点：不便于阅读，不便于异常处理

- 回调地狱最终解决方案：async / await
...
> 等同于 `new Promise((resolve, reject) => reject(error))`
...
只要 p1、p2、p3 中有一个实例对象率先改变状态，p 的状态就跟着改变
...
- resolve(value)：pendding  =>  fulfilled

- reject(error)：pendding  =>  rejected

- 抛出异常：pendding  =>  rejected

### 执行多个回调

当 Promise 的状态改变时，对应的回调函数都会执行
...
`promise.then()` 返回的新 Promise 的结果由 `then()` 的回调函数执行的结果决定

回调函数返回非 Promise 的任意值，`then()` 返回的新 Promise 状态变为 fulfilled，value 为返回值
...
回调函数返回 Promise，该 Promise 的结果就会成为 `then()` 返回的新 Promise 的结果
...
回调函数抛出异常，`then()` 返回的新 Promise 状态变为 rejected，error 为抛出的异常
...
new Promise(resolve => {
  ...
  .then(value => {
    ...
    return 2 // 同步操作可以返回非 Promise 的任意值
  })
  .then(value => {
    console.log("任务2的结果: ", value) // 2
    return new Promise(resolve => { // 异步操作需要返回一个 Promise 对象
      setTimeout(() => {
  ...
  .then(value => {
    console.log("任务3的结果: ", value) // 3
  })
```

### 异常穿透

使用 `then()` 的链式调用，在最后指定失败的回调，前面操作出现异常会逐层传递到 `catch()`
...
new Promise((resolve, reject) => {
  ...
  // error => { throw error } 或 error => Promise.reject(error)
).then(
  value => {
    console.log(value)
    return 3
...
Promise 回调、MutationObserver 回调

### 执行顺序

1. 先执行同步任务，再执行异步任务

2. 每取出一个宏任务执行之前，都要依次取出所有的微任务执行
