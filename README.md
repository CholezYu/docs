...
- 还可以使用 canvas 绘制图形，我在项目中使用过它的一个封装库 echarts，做大屏数据可视化
...
typeof：简单地判断数据类型，但是无法区分 null、object、array

instanceof：判断构造函数的原型对象是否在实例对象的原型链上

Object.prototype.toString.call：可以精确判断所有数据类型

## 说说常见的数组方法

遍历方法：forEach、filter、map、reduce、find、findIndex、every、some

更新方法：push、pop、unshift、shift、splice、reverse、sort

其他方法：slice、join、concat、indexOf、includes
...
- 作用域链：
  ...
  - 查找变量时，先在自身作用域查找，再沿着作用域链依次向外层查找，最终找到全局作用域，如果都找不到就会报错

## 说说对闭包的理解

闭包是一个引用外部函数变量的 closure 对象，存在内部函数中，可以通过断点调试查看

产生闭包的条件有：函数嵌套、内部函数引用外部函数的变量、外部函数被调用

闭包可以延长变量的声明周期，可以在函数外部操作函数内部的数据

但是闭包也可能导致内存泄漏。要解决这个问题，可以将内部函数变成垃圾对象被回收，而由于闭包存在内部函数中，所以闭包也被回收了
...
- 特殊函数
  ...
  - 生命周期函数，this 指向 Vue 实例对象

## 说说原型和原型链

- 原型

  - 原型指的是两个原型对象，显示原型和隐式原型

  - 除了箭头函数，所有函数都有一个显示原型对象，这个原型对象上有一个 constructor 属性指向函数本身;

  - 还有一个 [[prototype]] 属性指向 Object 的原型对象

  - 每个实例对象都有一个隐式原型对象，它指向对应的构造函数的原型对象

- 原型链

  - 原型链是多个隐式原型对象形成的一条链。用来查找对象的属性
  
  - 查找对象的属性和方法时，先在对象自身上查找，再沿着原型链向外层查找，最终找到 Object 的原型对象
  
  - 如果在 Object 的原型对象上没有找到，就返回 undefined
...
- Promise 的状态：
  ...
  - 通过 resolve 或 reject 方法改变 Promise 的状态
...
- 深拷贝和浅拷贝一般都是用来复制对象或数组

- 浅拷贝会将原对象的属性复制到新对象中

  - 如果属性是基本类型，那么就会直接复制这个值；
  
  - 如果属性是引用类型，那么复制的就是它的地址，这样会导致修改新对象中的属性，原对象中对应的属性也会改变
  
- 深拷贝会递归地将原对象的属性复制到新对象中

  - 即使属性是引用类型，那么它也会指向一个新的地址，修改新对象中的属性就不会影响原对象中对应的属性
  
- 我在项目中一般使用展开运算符进行浅拷贝，使用 lodash 的 cloneDeep 方法进行深拷贝
