...
- 新增了新的语义化标签：
  ...
  - 还可以建立更多跳转到自己网站的链接，让爬虫多次访问；
  
  - 或者提高网站加载速度，让爬虫在一定时间内访问更多网页；
  
  - 还可以设置 meta 标签的 keword 和 description 属性，设置一些关键字和描述；
...
- 还提供了一些新的 API
  ...
  - 还有 canvas，用于绘制图形，我在项目中使用过它的一个封装库 echarts，做大屏数据可视化

  - 比较常用的还有本地存储，localStorage 和 sessionStorage

    - localStorage 是持久化存储；sessionStorage 是会话存储，浏览器关闭时会话结束，数据被清除
    
    - 可以使用 localStorage 做七天免登录，登录时将后端返回的 token 分别存储在 vuex 和 localStorage 中，存在 vuex 是为了可以快速访问，存在 localStorage 是为了永久化存储
    
    - 与 localStorage 相似的还有 cookie，由后端设置并返回给前端，浏览器会自动存储，向后端发送请求时，浏览器会自动携带 cookie
    
  - 还有 Web Worker，开启分线程，可以将大量计算工作交给分线程，优化性能
