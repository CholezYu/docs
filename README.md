...
- 新增了新的语义化标签：
  ...
  - 使用语义化标签可以使 html 结构更清晰；还有利于 SEO 搜索引擎优化。
...
- 还新增了本地存储 API：
  ...
  - 可以使用 localStorage 做免登录，也就是对 token 做一些处理：
    ...
    1. 登录时将获取的 token 存在本地，当然也要存在状态管理库中，为了快速访问。

    2. 发送请求时，会在请求拦截器中将 token 携带在请求头中，发送给服务器。

    3. 返回响应时，会在响应拦截器中判断 token 是否有效。一般是在失败的回调中判断响应状态码是否为 407，或者在成功的回调中判断功能状态码是否是 207。如果是的话，说明 token 无效或者过期了。
...
- 还有 Web Worker：
  ...
  - 具体实现是：
    ...
    1. 实例化一个 worker 对象，创建分线程同时执行一个 js 文件；
...
- src 会将资源下载到文档中，替换当前元素。例如 script、img 和 iframe 标签。

- href 是建立文档与资源的链接。例如 link 标签和 a 标签。

- 浏览器遇到 src 会暂停其他资源的下载，阻塞当前页面的渲染。这就是要把 script 标签放在底部的原因。

- 而浏览器遇到 href 并不会停止对当前文档的处理，不会阻塞页面渲染。
...
- 新增了伪类选择器和伪元素选择器，还有属性名选择器

- 圆角边框

- 不透明度，可以用来代替 `display: none;`，减少重绘重排
...
- CSS 预处理器就是让 CSS 具有一些像 JS 一样的编程特性。

- 我常用的是 Sass 和 Less，就是用它的代码嵌套，还有定义常量，混入和继承。

- 在项目中，会写一个 variable.scss 文件，可以定义一些主题颜色和字体大小等。

- 对于一些重复的样式代码，例如文本溢出的样式、清除浮动，可以在 mixins.scss 文件中定义一些混入语法。
...
- 给父元素设置相对定位，子元素设置绝对定位，让子元素的水平垂直偏移量都为 50%，并且水平垂直平移 -50%。
...
- `typeof`：
  ...
  可以判断大部分数据类型，但是无法区分 null、object、array。
...
- 封装一个方法：
  ...
  先判断值是不是 null，再通过 `Array.isArray` 判断是不是数组类型，然后返回 typeof 的结果。
...
- 它的流程是：
  ...
  - 监听 onreadystatechange 或 onload 事件，看它请求是否发送成功。
...
- 在请求拦截器中携带公共的请求参数 token；

- 在响应拦截器中处理请求结果：

  - 在成功的回调中，判断功能状态码，是 20000 就返回数据，不是的话就根据状态码返回错误信息。

  - 在失败的回调中，又有两种情况：

    服务器有响应，说明响应结果是失败的。根据响应状态码判断，比如 404 就是找不到、401 就是未授权等等。

    服务器没响应，说明请求发送失败。根据错误消息判断，如果是 timeout 就是请求超时，network 就是网络连接失败，canceled 就是请求取消了。
...
- 需求背景：
  ...
  无感刷新就是用来优化用户体验的，或者也可以做单点登录。

- 具体做法：

  后端会返回两个 token，一个是 Access token，有效期很短，比如半个小时，它是用来请求用户数据的；一个是 Refresh token，有效期较长，比如七天或者半个月，它是用来请求 Access token 的。我们请求数据时，是携带 Access token 发送的，如果返回的 code 为 401，说明 Access token 过期了，就需要携带 Refresh token 去请求 Access token，然后携带新的 Access token 重新请求数据。这样就可以实现无感刷新的效果。也可以设置定时刷新，就是每隔一段时间，去请求一次 Access token。
...
- 按需引入，全局注册：
  ...
  先在一个 ts 文件里面按需引入 echarts，包括引入核心包、引入图表和图表的类型、引入一些功能组件和组件的类型、引入布局和过渡模块、引入 Canvas 渲染器、引入提供类型注册的模块，并使用 use 对这些组件或者模块进行安装，完成对 ECharts 的全局注册。

- 封装组件：

  然后就可以封装一个通用的 ECharts 组件。通过 props 传入一些配置项，比如可选的宽高属性，和必填的图表配置，然后使用 withDefault 给可选属性设置一个默认值。然后在 mounted 生命周期中对图表进行实例化并渲染图表，因为在 setup 中是获取不到 DOM 实例或组件实例的，需要在挂载完成后才能获取到。

- 使用：

  然后就可以在组件中使用我们封装的 ECharts 组件了。只需要给 ECharts 组件传入所需的配置项就行了，主要是传入图表配置。如果要新增公共配置，直接在 ECharts 组件中增加配置属性就可以了。
