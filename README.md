...
- Proxy，Vue3 的响应式原理就是基于 Proxy 实现的：[Vue3 响应式原理★](#Vue3 响应式原理★)
...
- 处理异步结果：
  ...
  - Promise 实例对象的 then 方法和 catch 方法可以处理异步结果；
...
- then 的返回值：
  ...
  - 通常情况下，如果没有 return，then 会返回一个成功状态的 Promise，值为 undefined；
...
- 请求报文：
  ...
  - 请求体可以携带数据发送给服务器。一般用于 Post 请求，Get 请求的请求体为空。
