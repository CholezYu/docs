...
- 还新增了本地存储 API：
  ...
  - 可以使用 localStorage 做免登录，也就是对 token 做一些处理：
    ...
    3. 返回响应时，会在响应拦截器中判断 token 是否有效。一般是在失败的回调中判断响应状态码是否为 407，或者在成功的回调中判断业务状态码是否是 207。如果是的话，说明 token 无效或者过期了。
...
- keep-alive 的原理：
  ...
  在 mounted 阶段，使用 cacheVNode 方法将组件缓存起来，还要判断缓存列表长度是否超过设置的最大缓存数量，如果超过的话，就使用 LRU 算法，删除缓存列表的第一个组件。同时监听 include 和 exclude 数组，一旦它们发生变化，就删除不需要缓存的组件。
